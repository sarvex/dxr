...
class SchemaTable(object):
    ...
    def __init__(self, tblname, tblschema):
        ...
        for col in tblschema:
            if isinstance(tblschema, (tuple, list)):
            ...
            if col == '_key':
                ...
                prefix = '' if len(spec) <= 1 else f"{spec[1]}_"
                self.columns.append((f"{prefix}file_id", ["INTEGER", True]))
                self.columns.append((f"{prefix}file_line", ["INTEGER", True]))
                self.columns.append((f"{prefix}file_col", ["INTEGER", True]))
    ...
    def get_create_sql(self):
        ...
        for col, spec in self.columns:
            specsql = f'{col} '
            specsql += special_types[spec[0]] if spec[0][0] == '_' else spec[0]
        ...
        colstrs.extend(
            f'FOREIGN KEY ({spec[0]}) REFERENCES {spec[1]}({spec[2]})'
            for spec in self.fkeys
        )
        if self.key is not None:
            colstrs.append(f"PRIMARY KEY ({', '.join(self.key)})")
        sql += ',\n  '.join(colstrs)
        sql += '\n);\n'
        ...
        if self.needFileKey is True:
            ...
            sql += f"CREATE UNIQUE INDEX {self.name}_file_index on {self.name} (file_id, file_line, file_col{', extent_start, extent_end' if has_extents else ''});"
        return sql

    def get_insert_sql(self, args):
        colset = {col[0] for col in self.columns}
        unwanted = [key for key in args.iterkeys() if key not in colset]
        for key in unwanted:
            del args[key]

        return (
            f"INSERT OR IGNORE INTO {self.name} ({','.join(args.keys())}) VALUES ({','.join('?' for _ in range(0, len(args)))})",
            args.values(),
        )
